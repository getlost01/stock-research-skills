...
Read-only deep dive on one symbol. See `reference/READ-ONLY-POLICY.md` for the hard
read-only rule, and `reference/RESEARCH-STANDARDS.md` for the technical-analysis
...
2. **Pull data.**
   ...
   - Stock fundamentals: `fetch_stocks_fundamental_data`,
     ...
     sector/market-cap band) per `reference/RESEARCH-STANDARDS.md` and compare
   ...
   - Technicals: `fetch_technical_screener`,
     ...
     framework from `reference/RESEARCH-STANDARDS.md`, not just one indicator.
   - **News (mandatory):** `WebSearch` for recent news on this name —
     ...
     `reference/RESEARCH-STANDARDS.md`: date-anchored queries, last-30-days
...
4. **Form a view, with reasoning shown:**
   ...
   - The view must satisfy the Recommendation completeness checklist in
     `reference/RESEARCH-STANDARDS.md` — horizon, numeric basis, 2-3 specific key
...
5. **Present:** one-paragraph verdict up top, then the supporting detail
   ...
   `reference/RESEARCH-STANDARDS.md` disclosure block. Optional: use the Research
   Note template in `reference/REPORT-TEMPLATES.md` if the user wants a formal/
