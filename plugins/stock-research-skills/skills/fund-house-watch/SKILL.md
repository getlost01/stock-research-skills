...
Read-only AMC-level surveillance. See `reference/READ-ONLY-POLICY.md` for the hard
read-only rule and `reference/RESEARCH-STANDARDS.md` for news freshness rules
...
4. **Present:** a per-AMC table (AMC, user's ₹ exposure, funds held,
   ...
   House Watch template in `reference/REPORT-TEMPLATES.md`, saved under `reports/`
   if wanted.
