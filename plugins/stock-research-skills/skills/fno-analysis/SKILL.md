...
Read-only options/futures analysis. See `reference/READ-ONLY-POLICY.md` for the hard
read-only rule — this never places/modifies/cancels an order, including
for F&O. See `reference/RESEARCH-STANDARDS.md` for the disclosure block.
...
7. **Present:** current exposure summary (net delta/theta, notional) first,
   ...
   with the `reference/RESEARCH-STANDARDS.md` disclosure block when a view/strategy
   suggestion is given (not needed for a plain position lookup). Optional:
   use the F&O Position Review template in `reference/REPORT-TEMPLATES.md` for a
