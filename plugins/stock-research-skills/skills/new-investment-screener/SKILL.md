...
Read-only idea generation. See `reference/READ-ONLY-POLICY.md` for the hard read-only
rule and `reference/RESEARCH-STANDARDS.md` for the peer-comparison framework and
disclosure block.
...
7. **Present:** shortlist table (name, key metric(s) matching the brief,
   ...
   evaluate/place themselves. Close with the `reference/RESEARCH-STANDARDS.md`
   disclosure block. Optional: use the Screener Shortlist template in
   `reference/REPORT-TEMPLATES.md` — worth it once there are several candidates,
