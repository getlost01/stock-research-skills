...
other skills. See `reference/READ-ONLY-POLICY.md` for the hard read-only rule and
`reference/RESEARCH-STANDARDS.md` for the bond peer-framework and disclosure block
...
4. **Analyze** (see `reference/RESEARCH-STANDARDS.md` → Bonds):
...
6. **Present:** instrument facts (issuer, coupon, maturity, rating) first
   ...
   analysis, then the view. Close with the `reference/RESEARCH-STANDARDS.md`
   ...
   Bond Note template in `reference/REPORT-TEMPLATES.md` for a formal/saveable
