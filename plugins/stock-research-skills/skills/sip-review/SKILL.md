...
them on a cadence. See `reference/READ-ONLY-POLICY.md` for the hard read-only rule and
`reference/RESEARCH-STANDARDS.md` for the MF peer framework, data-efficiency rules,
...
2. **Per-fund scorecard** (peer framework from `reference/RESEARCH-STANDARDS.md`,
...
5. **Present:** one summary table first (fund, SIP ₹, vs. benchmark, 
   ...
   SIP Review template in `reference/REPORT-TEMPLATES.md`, saved under `reports/`
