...
Read-only, best-effort estimation. See root `CLAUDE.md` for the hard
...
5. **Present:** a summary table (holding, qty, cost basis, current value,
   ...
   standard `RESEARCH-STANDARDS.md` disclosure block — this is estimate
   territory twice over (data completeness *and* not being tax advice).
   Optional: use the Capital Gains Snapshot template in
   `REPORT-TEMPLATES.md`, which most users of this skill will actually
