...
See root `CLAUDE.md` for the hard read-only rule and `RESEARCH-STANDARDS.md`
...
4. **Flag what needs action.** Only holdings where the result (or an
   ...
   in `RESEARCH-STANDARDS.md`. Routine in-line results are one line each.
...
5. **Present:** upcoming calendar first (date, name, weight in
   ...
   Optional: Earnings Watch template in `REPORT-TEMPLATES.md`, saved
