...
Read-only market context. See root `CLAUDE.md` for the hard read-only
rule and `RESEARCH-STANDARDS.md` for news-sourcing rules/preferred outlets.
...
6. **Present:** a short "what matters today" summary first (especially
   ...
   Optional: use the Market Pulse template in `REPORT-TEMPLATES.md` only
