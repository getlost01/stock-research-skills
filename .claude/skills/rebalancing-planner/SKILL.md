...
Read-only rebalancing analysis. See root `CLAUDE.md` for the hard
read-only rule and `RESEARCH-STANDARDS.md` for the disclosure block
...
6. **Present:** a target-vs-actual table first, then concentration/overlap
   ...
   the `RESEARCH-STANDARDS.md` disclosure block. Optional: use the
   Rebalancing Plan template in `REPORT-TEMPLATES.md` for a formal/
