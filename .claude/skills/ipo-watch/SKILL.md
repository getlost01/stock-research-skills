...
Read-only IPO research. See root `CLAUDE.md` for the hard read-only rule —
this never applies to/subscribes for an IPO on the user's behalf. See
`RESEARCH-STANDARDS.md` for the peer-comparison framework, news-sourcing
...
3. **Benchmark against listed peers.** Use `fetch_fundamentals_screener` /
   ...
   the peer framework in `RESEARCH-STANDARDS.md`.
...
7. **Present:** key IPO facts up top (dates, price band, lot size), then
   ...
   the `RESEARCH-STANDARDS.md` disclosure block. Optional: use the IPO
   Note template in `REPORT-TEMPLATES.md` for a formal/saveable version —
