...
Read-only. See root `CLAUDE.md` for the hard read-only rule and
`RESEARCH-STANDARDS.md` for the peer-comparison framework and disclosure
...
3. **Peer comparison** (see `RESEARCH-STANDARDS.md` → Mutual funds/ETFs):
...
7. **Present:** fund summary (category, expense ratio, benchmark, AUM)
   ...
   view. Close with the `RESEARCH-STANDARDS.md` disclosure block. Optional:
   use the Fund Note template in `REPORT-TEMPLATES.md` for a formal/
